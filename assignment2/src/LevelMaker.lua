...
function LevelMaker.createMap(level)
    ...
    -- randomly choose the number of columns, ensuring odd (so they will be symmetrical in the screen)
    ...
    local highestColor = math.min(5, math.floor(level / 2))

    -- track if locked brick is already present in this map
    ...
    for y = 1, numRows do
        ...
        for x = 1, numCols do
            ...
            -- 1 in 50 chance this brick will be a locked brick (if none in map yet)
            if not lockedBrick and math.random(50) == 1 then
                lockedBrick = true
                b.locked = true
