...
public class LevelGenerator : MonoBehaviour {
	...
	void Start () {
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				// chance for a hole in the floor if it is a maze pathway
				if (!mapData[z, x] && holes < floorHoles && Random.value < 0.04) {
					holes += 1;
				} else {
					// create floor
	...
	bool[,] GenerateMazeData() {
		...
		// iterate our random crawler, clearing out walls
