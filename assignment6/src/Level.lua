...
function Level:init()
    ...
    function beginContact(a, b, coll)
        ...
        if types['Obstacle'] and types['Player'] then
            ...
            -- player has collided
            self.launchMarker.playerCollided = true
...
function Level:update(dt)
    ...
    -- replace launch marker if all player's aliens stopped moving
    if self.launchMarker.launched then
        -- whether any of player's aliens still exist
        local exist = false

        -- test all player's aliens (that are not destroyed yet)
        for k, playerAlien in pairs(self.launchMarker.aliens) do
            if not playerAlien.body:isDestroyed() then
                -- as long as any player's aliens are still present, this is true
                exist = true

                local xPos, yPos = playerAlien.body:getPosition()
                local xVel, yVel = playerAlien.body:getLinearVelocity()
                
                -- if we fired our alien to the left or it's almost done rolling, destroy
                if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 5) then
                    playerAlien.body:destroy()
                end
            end
        end

        -- when all player's aliens are gone
        if not exist then
            self.launchMarker = AlienLaunchMarker(self.world)
