...
function AlienLaunchMarker:init(world)
    ...
    -- whether the first alien (player) has collided with anything
...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        -- launch 2 more player aliens
        if love.keyboard.wasPressed('space') and #self.aliens == 1 and not self.playerCollided then
            local pX, pY = self.aliens[1].body:getPosition()
            local vX, vY = self.aliens[1].body:getLinearVelocity()
            local angleShift = {20, -20}

            for i = 2, 3 do
                self.aliens[i] = Alien(self.world, 'round', pX, pY, 'Player')
                self.aliens[i].body:setLinearVelocity(vX, vY + angleShift[i - 1])
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        -- render aliens that are not 'dead'
        for i = #self.aliens, 1, -1 do
            if not self.aliens[i].body:isDestroyed() then
