...
function TakeTurnState:attack(attacker, defender, attackerSprite, defenderSprite, attackerkBar, defenderBar, onEnd)
    ...
    Timer.after(0.5, function()
        ...
        :finish(function()
            ...
            Timer.every(0.1, function()
                defenderSprite.opacity = defenderSprite.opacity == 64 and 1 or 64
            end)
            :limit(6)
            :finish(function()
                ...
                :finish(function()
                    -- actually change the value of the defender's health bar
            ...
            Timer.after(1.5, function()
                ...
                :finish(function()
                    ...
                    if self.playerPokemon.currentExp > self.playerPokemon.expToLevel then
                        ...
                        -- level up & get the increment in values of stats
                        local incHP, incAtk, incDef, incSpd = self.playerPokemon:levelUp()

                        gStateStack:push(BattleMessageState('Congratulations! Level Up!',
                        
                        function()
                            -- menu to show stat data on level up
                            gStateStack:push(MenuState(incHP, incAtk, incDef, incSpd, self.playerPokemon,
                            function()
                                self:fadeOutWhite()
                            end))
                        end))
